...
export function surveyRoutes(service: SaywideService, config: AppConfig): FastifyPluginAsync {
  return async (app) => {
    ...
    typed.post("/api/surveys", {
      ...
      const survey = await service.createSurvey(guest.organizerId, request.body, idempotencyKey(request));
    ...
    typed.patch("/api/surveys/:surveyId", {
      ...
      requireAllowedOrigin(request, config);
      const guest = await service.requireGuest(request.cookies[GUEST_COOKIE]);
      return service.updateSurvey(guest.organizerId, request.params.surveyId, request.body);
    });
    ...
    typed.post("/api/surveys/:surveyId/publish", {
      ...
      requireAllowedOrigin(request, config);
      const guest = await service.requireGuest(request.cookies[GUEST_COOKIE]);
      const survey = await service.publishSurvey(guest.organizerId, request.params.surveyId);
      return {
    ...
    typed.post("/api/surveys/:surveyId/close", {
      ...
      requireAllowedOrigin(request, config);
      const guest = await service.requireGuest(request.cookies[GUEST_COOKIE]);
      const survey = await service.closeSurvey(guest.organizerId, request.params.surveyId);
    ...
    typed.post("/api/surveys/:surveyId/reopen", {
      ...
      requireAllowedOrigin(request, config);
      const guest = await service.requireGuest(request.cookies[GUEST_COOKIE]);
      const survey = await service.reopenSurvey(guest.organizerId, request.params.surveyId);
      return {
        surveyId: survey.surveyId,
        status: "open" as const,
        shareUrl: `${config.publicAppUrl}${survey.participantUrl}`,
