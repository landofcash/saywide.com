...
export function WelcomeScreen({ publicToken }: { publicToken: string }) {
  ...
  return (
    <ParticipantShell>
      <div className="text-center"><p className="text-xs font-bold uppercase tracking-[0.16em] text-[var(--coral-dark)]">You have been invited to share</p><h1 className="font-display mx-auto mt-4 max-w-2xl text-3xl font-bold tracking-[-0.035em] sm:text-4xl">{survey.title}</h1><p className="mx-auto mt-4 max-w-xl text-base leading-7 text-[var(--muted)] sm:text-lg">{survey.introduction}</p></div>
