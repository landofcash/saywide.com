...
export function ReviewScreen({ publicToken }: { publicToken: string }) {
  ...
  return (
    <ParticipantShell>
      ...
      <div className="mt-3"><p className="text-xs font-bold uppercase tracking-[0.16em] text-[var(--coral-dark)]">Final check</p><h1 className="font-display mt-3 text-5xl font-bold tracking-[-0.05em]">Review your words</h1><p className="mt-4 text-lg leading-8 text-[var(--muted)]">This is exactly the text that will be stored. You can go back and change anything before submitting.</p></div>
      <div className="mt-7 space-y-4">{survey.questions.map((question, index) => <Card key={question.questionId} className="p-5 sm:p-6"><div className="flex items-start justify-between gap-4"><div className="min-w-0"><p className="text-xs font-bold uppercase tracking-[0.12em] text-[var(--muted)]">Question {index + 1}</p><h2 className="mt-2 font-bold leading-6">{question.prompt}</h2></div><Button variant="ghost" size="sm" asChild><Link href={`/s/${publicToken}/respond?question=${index + 1}`}><Pencil className="size-4" /> Edit</Link></Button></div><p className={`mt-4 whitespace-pre-line rounded-2xl p-4 text-sm leading-7 ${answers[question.questionId] ? "bg-[var(--canvas)]" : "bg-red-50 text-red-800"}`}>{answers[question.questionId] || (question.required ? "Required answer missing" : "No answer")}</p></Card>)}</div>
      {error && <p className="mt-5 rounded-xl bg-red-50 p-3 text-sm text-red-800" role="alert">{error}</p>}
      <Card className="mt-6 border-emerald-200 bg-[var(--mint-soft)] p-5"><div className="flex gap-3"><CheckCircle2 className="mt-0.5 size-5 shrink-0 text-emerald-800" /><p className="text-sm leading-6 text-emerald-950/80">I understand my reviewed text will be submitted anonymously and may appear as a short, pseudonymous evidence excerpt in the organizer&apos;s report.</p></div></Card>
