...
export function Brand({ compact = false, className }: { compact?: boolean; className?: string }) {
  return (
    <Link href="/" className={cn("group inline-flex items-center gap-3", className)} aria-label="Saywide home">
      <span className="relative grid size-10 place-items-center rounded-full bg-[var(--ink)] text-white">
      ...
      {!compact && <span className="font-display text-2xl font-bold tracking-[-0.04em]">saywide</span>}
    </Link>
  );
}
