...
function AccountFrame({ eyebrow, title, description, children }: { eyebrow: string; title: string; description: string; children: React.ReactNode }) {
  return (
    <div className="noise min-h-screen px-5 py-7 sm:py-12">
      <div className="mx-auto max-w-5xl"><Brand /><div className="mt-8 grid gap-8 lg:grid-cols-[0.85fr_1.15fr] lg:items-center"><section><p className="text-xs font-bold uppercase tracking-[0.16em] text-[var(--coral-dark)]">{eyebrow}</p><h1 className="font-display mt-3 text-5xl font-bold tracking-[-0.055em] sm:text-6xl">{title}</h1><p className="mt-5 max-w-md text-lg leading-8 text-[var(--muted)]">{description}</p><Link href="/" className="mt-7 inline-flex min-h-11 items-center gap-1 text-sm font-semibold text-[var(--muted)]"><ArrowLeft className="size-4" /> Back to survey creation</Link></section>{children}</div></div>
    </div>
  );
}
...
export function ClaimAccountScreen() {
  ...
  return <AccountFrame eyebrow="One clear choice" title="Bring your guest surveys with you?" description="This browser has work that is not yet connected to your signed-in account. Nothing moves unless you confirm it."><Card className="p-6 sm:p-8"><div className="rounded-2xl bg-[var(--mint-soft)] p-5"><p className="font-display text-3xl font-bold">3 guest surveys</p><p className="mt-2 text-sm leading-6 text-emerald-950/70">They will appear beside your account surveys. The guest access on this browser will then be retired.</p></div><label className="mt-6 flex cursor-pointer gap-3 rounded-2xl border border-[var(--line)] p-4 text-sm leading-6"><input type="checkbox" checked={confirmed} onChange={(event) => setConfirmed(event.target.checked)} className="mt-1 size-4 shrink-0 accent-[var(--ink)]" /><span><strong className="block">Move these surveys into my account</strong>I understand this changes their owner and cannot be undone from this screen.</span></label>{success && <p className="mt-4 rounded-xl bg-emerald-50 p-3 text-sm font-semibold text-emerald-800" role="status">{success}</p>}<Button variant="accent" size="lg" className="mt-6 w-full" disabled={!confirmed || loading} onClick={claim}>{loading ? "Moving surveys…" : "Move surveys to my account"}</Button><Button variant="ghost" className="mt-2 w-full" asChild><Link href="/dashboard">Not now</Link></Button></Card></AccountFrame>;
}
