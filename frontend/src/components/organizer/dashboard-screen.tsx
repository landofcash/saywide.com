    ...
    <OrganizerShell wide>
      <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
        <div>
          ...
          <h1 className="font-display mt-2 text-5xl font-bold tracking-[-0.05em] sm:text-6xl">My surveys</h1>
      ...
      <Card className="mt-8 flex flex-col gap-4 border-emerald-200 bg-[var(--mint-soft)] p-5 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex gap-3">
          <span className="grid size-11 shrink-0 place-items-center rounded-full bg-white"><ShieldCheck className="size-5 text-emerald-800" /></span>
      ...
      <div className="mt-8 flex gap-2 overflow-x-auto pb-2" aria-label="Filter surveys">
        {filters.map((item) => (
          <button key={item} onClick={() => setFilter(item)} className={`min-h-10 rounded-full border px-4 text-sm font-semibold capitalize ${filter === item ? "border-[var(--ink)] bg-[var(--ink)] text-white" : "border-[var(--line)] bg-white/60 text-[var(--muted)]"}`}>
            {item}
          </button>
        ))}
      </div>
      ...
      {loading ? (
        <div className="mt-5 grid gap-4 lg:grid-cols-3" aria-label="Loading surveys">{[1, 2, 3].map((item) => <div key={item} className="h-64 animate-pulse rounded-[1.75rem] bg-white/60" />)}</div>
      ) : visible.length === 0 ? (
          ...
          {visible.map((survey) => {
            ...
            return (
              <Link href={href} key={survey.surveyId} className="group block">
                <Card className="flex h-full min-h-64 flex-col p-6 transition duration-200 group-hover:-translate-y-1 group-hover:border-[var(--ink)] group-hover:shadow-[0_22px_60px_rgba(36,48,44,0.12)]">
                  ...
                  <div className="mt-auto grid grid-cols-2 gap-3 pt-8 text-sm">
                    <div className="rounded-2xl bg-[var(--canvas)] p-3"><p className="text-2xl font-bold">{survey.submittedResponseCount}</p><p className="text-xs text-[var(--muted)]">submissions</p></div>
                    <div className="rounded-2xl bg-[var(--canvas)] p-3"><p className="text-2xl font-bold">{survey.questionCount}</p><p className="text-xs text-[var(--muted)]">questions</p></div>
                  </div>
